...
var instance = new Razorpay({
    key_id: 'rzp_test_FkBD5KXrCWE30C',
    key_secret: 'saie2ABqzadmcVor2CaqFHmn',
  });
module.exports={
    doSignup:(userData)=>{
...
verifyPayment:(details)=>{
  return new Promise((resolve,reject)=>{
      ...
      let hmac = crypto.createHmac('sha256', 'saie2ABqzadmcVor2CaqFHmn')
...
takeAppoinment:(appointment,callback)=>{
...
deleteProduct:()=>{
  return new Promise((resolve,reject)=>{
      db.get().collection(collection.CART_COLLECTION).removeOne({_id:objectId()}).then((response)=>{
...
deleteUserOrder:(users)=>{
    return new Promise((resolve,reject)=>{
        db.get().collection(collection.ORDER_COLLECTION).removeOne({_id:objectId(users)}).then((response)=>{
            //console.log(response);
            resolve(response)
        })
    })
