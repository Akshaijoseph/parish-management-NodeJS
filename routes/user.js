...
router.post('/verify-payment',(req,res)=>{
 ...
 router.get('/take-appointment',verifyLogin,(req,res)=>{
   if(req.session.user){
      res.render('user/take-appointment',{user:req.session.user})
    }else{
       res.redirect('/login')
    }
...
router.post('/take-appointment',verifyLogin,(req,res)=>{
   console.log(req.body)
   userHelpers.takeAppoinment(req.body,(result)=>{
      res.render('user/take-appointment',{user:req.session.user})  
   })
})
...
router.get('/delete-product/:id',(req,res)=>{
   ...
   console.log()
   productHelpers.deleteProduct().then((response)=>{
     res.redirect('/cart/')
   })
...
router.get('/more-info',(req,res)=>{
   if(req.session.user){
   res.render('user/more-info',{user:req.session.user})
 }else{
    res.redirect('/login')
 }
})
router.get('/info-view', function(req, res, next) {
   userHelpers.getAllinfo().then((users)=>{
      console.log(users)
     res.render('user/info-view',{user:req.session.user,users});
   })
